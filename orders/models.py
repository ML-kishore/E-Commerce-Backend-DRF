...
class Orders(models.Model):
    ...
    payment_choices = (('success','SUCCESS'),('failed','FAILED'),('pending','PENDING'),('refunded','REFUNDED'))
    user = models.ForeignKey(User,on_delete=models.PROTECT,related_name='user_order')
    order_date = models.DateTimeField(auto_now_add=True)
    order_status = models.CharField(default='order_placed',max_length=20,choices=order_choices)
    amount = models.DecimalField(max_digits=10,decimal_places=2)
    is_cancelled = models.BooleanField(default=False)
    payment_status = models.CharField(default='success',max_length=10,choices=payment_choices)
